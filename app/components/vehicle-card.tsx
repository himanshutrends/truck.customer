...
export const VehicleCard: React.FC<VehicleCardProps> = ({
  ...
  const selectionCheck = canSelectVehicle(vehicle);

  const handleSelectionChange = (checked: boolean) => {
    if (checked) {
      addVehicleToQuotation(vehicle);
    } else {
      removeVehicleFromQuotation(vehicle.id);
    }
  };
            ...
            <TooltipContent>
              <div className="max-w-xs">
                {selectionCheck.reason ? (
                  <div>
                    <p className="font-medium text-amber-600 mb-1">Vendor Switch Warning</p>
                    <p className="text-xs">{selectionCheck.reason}</p>
                  </div>
                ) : (
                  <p>Add to quotation</p>
                )}
              </div>
            </TooltipContent>
          </Tooltip>
