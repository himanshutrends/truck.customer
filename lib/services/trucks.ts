...
import { TruckType, TruckSearchResult, TruckSearchParams } from '../types';
...
export class TrucksService {
  ...
  static async getTruckTypes(): Promise<TruckType[]> {
    try {
      const response = await ApiHandler.getTruckTypes();
   ...
   */
  static async searchTrucks(searchParams: QuotationSearchParams): Promise<VehicleDetails[]> {
    try {
      ...
      const truckSearchParams: TruckSearchParams = {
        ...
        weight: this.extractWeightNumber(searchParams.weight, searchParams.weightUnit),
        capacity_min: this.extractWeightNumber(searchParams.weight, searchParams.weightUnit),
        capacity_max: this.extractWeightNumber(searchParams.weight, searchParams.weightUnit) * 1.5, // Allow 50% more capacity
        availability_status: 'available',
        limit: 50, // Default limit
      };

      const response = await ApiHandler.searchTrucks(truckSearchParams);
      console.log('Search results:', response);
      if (response.success && response.data && response.data.trucks) {
        return this.transformTruckResultsToVehicleDetails(
   ...
   */
  private static transformTruckResultsToVehicleDetails(
    ...
    return trucks.map((truck, index) => {
      ...
      return {
        ...
        badge: this.generateBadge(truck, index),
   ...
   */
  private static extractWeightNumber(weight: string, unit: 'kg' | 'tonnes'): number {
   ...
   */
  private static generateBadge(truck: TruckSearchResult, index: number): string | undefined {
